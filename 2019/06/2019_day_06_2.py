...
COM - B - C - D - E - F
               \
                I - SAN
In this example, YOU are in orbit around K, and SAN is in orbit around I. To move from K to I, a minimum
of 4 orbital transfers are required:
...
def pathfind_tree( start, goal ):
	# Build list of bodies walking from start up to root/COM
	start_path = [ ]
	cur_body = start

	while cur_body.name != 'COM':
		start_path.append( cur_body )
		cur_body = cur_body.orbits

	start_path.append( cur_body )

	# Now walk up from goal until we hit a body from first list
	goal_path = [ ]
	cur_body = goal

	while cur_body not in start_path:
		goal_path.append( cur_body )
		cur_body = cur_body.orbits

	goal_path.append( cur_body )

	# Build path to each other
	path = start_path[ :start_path.index( goal_path[ -1 ] ) + 1 ] + list( reversed( goal_path[ :-1 ] ) )
...
if __name__ == "__main__":
	...
	path = pathfind_tree( bodies[ 'YOU' ], bodies[ 'SAN' ] )

	print( 'min transfers =', len( path ) - 3 )
