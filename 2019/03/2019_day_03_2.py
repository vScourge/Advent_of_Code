...
for this measurement. While the wires do technically cross right at the central port where they both start, this p
oint does not count, nor does a wire count as crossing with itself.
...
if __name__ == "__main__":
	...
	for coord1 in wire1.path:
		print( '{0} of {1} - {2}'.format( c, len( wire1.path ), fewest_steps ) )
		c += 1
		
		if coord1 == ( 0, 0 ) or coord1 not in wire2.path:
			continue
		
		steps = wire1.path.index( coord1 ) + wire2.path.index( coord1 )
