...
def get_params( codes, i, relative_base, param_modes ):
	...
	for mode in param_modes:
		if mode == MODE_POSITION:
			...
			#idx = codes.get( codes.get( i, 0 ) + relative_base, 0 )
			#print( 'idx =', idx )
			#params.append( codes.get( idx, 0 ) )
...
class Computer( ):
	...
	def run( self ):
		if os.path.exists(r'D:\projects\ctg\main\out1.txt' ):
			os.remove( r'D:\projects\ctg\main\out1.txt' )
		...
		while True:
			...
			with open( r'D:\projects\ctg\main\out1.txt', 'a' ) as out_file:
				out_file.write( ','.join( [ str( x ) for x in codes.values( ) ] ) + '\n' )
