...
player keeps the marble they would have placed, adding it to their score. In addition, the marble 7 marbles counter-clockwise from the current marble is
removed from the circle and also added to the current player's score. The marble located immediately clockwise of the marble that was removed becomes the new current marble.
...
if __name__ == "__main__":
	...
	while True:
		...
		if marble % 23 == 0:
			...
			players[ cur_player_idx ] += marble + marble2
		...
		cur_player_idx += 1
		if cur_player_idx > num_players:
			cur_player_idx = 1

		#print( 'M{0} P{1} {2}'.format( marble, cur_player_idx, circle ) )

		if marble == last_marble_value:
			high_score = sorted( players.values( ) )[ -1 ]
