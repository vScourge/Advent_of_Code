...
if __name__ == "__main__":
	...
	circle = numpy.array( [ cur_marble, cur_marble + 1 ] )
	...
	while True:
		...
		if next_marble_num % 100000 == 0:
			print( 'marble_num = {0}'.format( next_marble_num ) )
		...
		if marble % 23 == 0:
			...
			if cur_marble_idx < 0:
				cur_marble_idx = len( circle ) - -cur_marble_idx

			circle = numpy.delete( circle, cur_marble_idx )
			...
			circle = numpy.insert( circle, insert_idx, marble )
		...
		#print( 'M{0} P{1} {2} - {3}'.format( marble, cur_player_idx, circle, cur_marble_idx ) )
