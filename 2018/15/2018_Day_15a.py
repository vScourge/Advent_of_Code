...
class Manager( ):
	...
	def pathfind( self, start, goal ):
		...
		while True:
			...
			new_paths = sorted( new_paths, key = lambda p: self.get_distance( p[ -1 ], goal ) )
	...
	def get_goal_path( self, unit ):
		...
		while e < len( enemies ):
			...
			## See if we have any valid paths
			#if paths:
				## Get path with shortest length
				#valid_path = paths[ sorted( paths.keys( ) )[ 0 ] ][ 0 ]
...
if __name__ == "__main__":
	...
	while not manager.match_over( ):
		...
		for unit in manager.get_units( ):
			print( 'unit {0}/{1} - {2:.2f}%'.format( unit_count, len( manager.units ), unit_count / len( manager.units ) ) )
