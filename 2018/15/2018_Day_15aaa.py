...
class Manager( ):
	...
	def pathfind( self, start, goal ):
		...
		for cluster in _Cluster_cache:
			if start in cluster and goal in cluster:
				break
			
		if not cluster:
			cluster = self._get_links_recurse( start )
			_Cluster_cache.add( tuple( set( cluster ) ) )
			
			if goal not in cluster:
				# Cannot be reached, blocked by other unit(s)
				return [ ]
	...
	def _get_links_recurse( self, spot, cluster = [ ] ):
		cluster.append( spot )
		new_links = list( set( [ s for s in spot.links.values( ) if s and s not in cluster and s.contents == EMPTY ] ) )
		cluster += new_links
		cluster = list( set( cluster ) )
		
		for link in new_links:
			cluster += self._get_links_recurse( link, cluster = cluster )
