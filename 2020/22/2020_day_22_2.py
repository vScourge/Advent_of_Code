...
value of the card multiplied by 1, the second-from-the-bottom card is worth the value of the card multiplied 
by 2, and so on. With 10 cards, the top card is worth the value on the card multiplied by 10. In this example, 
...
from other games are not considered. (This prevents infinite games of Recursive Combat, which everyone agrees is a bad idea.)
...
cards dealt at the beginning of the round and places them on the bottom of their own deck (again so that the winner's 
card is above the other card). Note that the winner's card might be the lower-valued of the two cards if they won the 
...
def main( deck1, deck2 ):
	...
	while len( deck1 ) and len( deck2 ):
		print( '\nRound', c )
		print( 'deck 1 =', ', '.join( [ str( x ) for x in deck1 ] ) )
		print( 'deck 2 =', ', '.join( [ str( x ) for x in deck2 ] ) )

		history_entry = ( tuple( deck1 ), tuple( deck2 ) )
		if history_entry in history:
			winner = 1
			break

		# Add new hands to history
		...
		if len( deck1 ) >= card1 and len( deck2 ) >= card2:
			...
			# Play non-recurse Combat like usual
			if card1 > card2:
				deck1.append( card1 )
				deck1.append( card2 )
			else:
