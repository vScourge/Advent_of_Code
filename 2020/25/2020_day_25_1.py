...
door and then inject your own commands into the data stream, but that's definitely impossible." You thank them 
for their time.
...
Transforming the subject number of 17807724 (the door's public key) with a loop size of 8 (the card's loop size) produces 
the encryption key, 14897079. (Transforming the subject number of 5764801 (the card's public key) with a loop size of 11 
...
def get_loop_size( subject_num, public_key ):
	divisor = 20201227
	value = 1
	c = 0
...
def main( numbers ):
	...
	card_public_key, door_public_key = numbers

	card_loop_size = get_loop_size( subject_num, card_public_key )
	print( 'card_loop_size =', card_loop_size )
	
	door_loop_size = get_loop_size( subject_num, door_public_key )
