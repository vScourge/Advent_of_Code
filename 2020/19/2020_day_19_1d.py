...
def parse_input( ):
	...
	for line in lines:
		...
		if ':' in line:		
			...
			if '"' in line:
				letter = parts1[ -1 ][ 1 ]
				code = [ 'out = "{0}".find( "{1}" )'.format( '{msg}', letter ) ]
				...
				for part in parts2.split( '|' ):
					...
					if len( ids ) == 1:
						code.append( 'out = rules[ {0} ]( "{1}" )'.format( ids[ 0 ], '{msg}' ) )
					elif len( ids ) == 2:
						#code = 'a = rules[ {0} ]( "{1}" );out = a >= 0 and rules[ {2} ]( "{1}"[ a+1: ] )'.format( ids[ 0 ], '{msg}', ids[ 1 ] )
						code.append( 'a = rules[ {0} ]( "{1}" );out = rules[ {2} ]( "{1}"[ a+1: ] ) if a >= 0 else -1'.format( ids[ 0 ], '{msg}', ids[ 1 ] ) )
					elif len( ids ) == 3:
						#code = 'a = rules[ {0} ]( "{1}" );b = a >= 0 and rules[ {2} ]( "{1}"[ a+1: ] );out = b >= 0 and rules[ {3} ]( "{1}"[ b+1: ] )'.format( ids[ 0 ], '{msg}', ids[ 1 ], ids[ 2 ] )
						code.append( 'a = rules[ {0} ]( "{1}" );b = rules[ {2} ]( "{1}"[ a+1: ] ) if a >= 0 else -1;out = rules[ {3} ]( "{1}"[ b+1: ] ) if b >= 0 else -1'.format( ids[ 0 ], '{msg}', ids[ 1 ], ids[ 2 ] ) )
					else:
						raise IOError( 'doh' )
...
class Rule( ):
	...
	def __call__( self, message ):
		...
		for code_line in self.code:
			code_str = code_line.format( msg = message )
			print( code_str )
