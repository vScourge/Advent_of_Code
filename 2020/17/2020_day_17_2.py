...
def parse_input( ):
	...
	# Add the 4th axis, it's now a 4D grid
	grid = grid[ numpy.newaxis, : ]
...
def get_neighbors( grid, coord ):
	"""
	Slice out a chunk of the grid that surrounds the
	supplied coordinate.
	"""
...
def main( grid ):
	cycles = 6
	grid = numpy.pad( grid, pad_width = cycles+1, mode = 'constant', constant_values = 0 )
			...
			for z in range( 1, len( grid[ w ] ) - 1 ):
				for y in range( 1, len( grid[ w ][ z ] ) - 1 ):
					for x in range( 1, len( grid[ w ][ z ][ y ] ) - 1 ):
						...
						# Grab the slice of the grid that's 1 unit away from this coord
