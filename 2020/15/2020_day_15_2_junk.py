...
def main( ):
	...
	for num in numbers:
		data[ num ] = collections.deque( [ turn ], maxlen = 2 )
		turn += 1

	cur_num = num
		
	while True:
		if len( data[ cur_num ] ) == 1:
			num = 0
		else:
			num = data[ cur_num ][ -1 ] - data[ cur_num ][ -2 ]

			if num not in data:
				data[ num ] = collections.deque( [ ], maxlen = 2 )

		data[ num ].append( turn )
		cur_num = num
