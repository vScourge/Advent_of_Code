...
if __name__ == "__main__":
	...
	ids = [ int( l ) for l in lines[ 1 ].split( ',' ) if l != 'x' ]
	ids.sort( )
	...
	for id in ids:
		...
		while val <= depart_time:
			val += id
			i += 1
			
		wait_times[ val - depart_time ] = id
	
	shortest_time = sorted( list( wait_times.keys( ) ) )[ 0 ]
	answer = shortest_time * wait_times[ shortest_time ]
