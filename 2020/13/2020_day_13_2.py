...
0, 11, 22, 33, and so on. If you are there when the bus departs, you can ride that bus to the airport!
...
bus IDs can depart at other times, and other bus IDs can depart at those times. For example, in 
the list above, because bus ID 19 must depart seven minutes after the timestamp at which bus ID 7 
...
if __name__ == "__main__":
	...
	ids_str = lines[ 1 ].split( ',' )
	
	ids = { }

	for i in range( len( ids_str ) ):
		if ids_str[ i ] == 'x':
			# change these to int so they're not so annoying
			ids[ i ] = -1
		else:
			ids[ i ] = int( ids_str[ i ] )
	
	id_indices = list( ids.keys( ) )

	num_ids = 2		# How many bus IDs to start with when checking condition
	
	# This is our working list of bus IDs that we're currently testing agains.
	# It starts with just the first two busses.
	cur_ids = numpy.array( [ ids[ i ] for i in id_indices[ :num_ids ] ], dtype = 'int64' )

	c = 0
	step = 1
	
	while True:
		# See if all of them have modulus of 0 OR a value of -1 (which we ignore)
		if all( [ ids[ i ] == -1 or ( c + i ) % ids[ i ] == 0 for i in range( num_ids ) ] ):
			if num_ids == len( ids ):
				# We're tested all bus IDs, so we have a winner
				break
			
			# Get lowest common denominator for all our IDs
			step = numpy.lcm.reduce( cur_ids )

			# Increment the end of our range until we get the next valid bus ID
			# (ignoring -1 values)
			num_ids += 1
			while ids[ num_ids - 1 ] == -1:
				num_ids += 1
				
			# Add the next bus ID to our list
			cur_ids = numpy.array( [ ids[ i ] for i in id_indices[ :num_ids ] ], dtype = 'int64' )
