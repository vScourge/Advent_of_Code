...
class Passport( ):
	...
	def validate( self ):
		...
		if not self.birth_year or len( self.birth_year ) != 4:
			return
		...
		if not self.issue_year or len( self.issue_year ) != 4:
			return
		
		self.issue_year = int( self.issue_year )

		if self.issue_year < 2010 or self.issue_year > 2020:
			return
		
		if not self.exp_year or len( self.exp_year ) != 4:
			return
		
		self.exp_year = int( self.exp_year )

		if self.exp_year < 2020 or self.exp_year > 2030:
			return
		
		if not self.height:
			return
		
		if not self.height.endswith( 'cm' ) and not self.height.endswith( 'in' ):
			return
		
		val = int( self.height[ :-2 ] )
		...
		if not self.hair_color or len( self.hair_color ) != 7:
			return
		
		if not self.eye_color or self.eye_color not in ( 'amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth' ):
			return
		
		if not self.passport_id or len( self.passport_id ) != 9:
			return
