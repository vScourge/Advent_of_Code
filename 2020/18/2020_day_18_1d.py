...
def eval_recurse( val ):
	...
	"""
	# Find first inner-most set of parens
	i = 0
	open_pos = 0
	
	while i < len( val ):
		if '(' not in val:
			# Done walking, return eval
		...
		if '(' in val:
			if val[ i ] == '(':
				...
				#val2 = eval_recurse( val[ start+1:end ] )
				val2 = eval_recurse( val[ start:end ] )
				val = val[ :start ] + str( val2 ) + val[ end + 1: ]
			else:
				i += 1
...
class Int_New( ):
	...
	def __mul__( self, other ):
		return Int_New( self.value * other.value )
	...
	def __repr__( self ):
		return '<Int_New {0}>'.format( self.value )
