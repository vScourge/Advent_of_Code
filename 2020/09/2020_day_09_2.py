...
clever use of several paperclips. Upon connection, the port outputs a series of numbers (your puzzle input).
...
should be the sum of any two of the 25 immediately previous numbers. The two numbers will have different 
...
if __name__ == "__main__":
	# STEP 1 - find the invalid number
	...
	while True:
		chunk = data[ i - PREAMBLE_SIZE:i ]		# range to check
		value = data[ i ]						# Target number
		match_found = False
		
		#print( '---')
		#print( 'i = {0} ({1})'.format( i, value ) )
	...
	for i in range( len( data ) - 1 ):
		...
		while True:
			vals = data[ i : i + run_length ]

			if sum( vals ) == invalid_number:
				vals.sort( )
				answer = sum( ( vals[ 0 ], vals[ -1 ] ) )
				print( 'answer = {0}'.format( answer ) )
				sys.exit( )
				
			run_length += 1

			if run_length >= len( data ):
				break
