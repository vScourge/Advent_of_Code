...
def get_branches_recurse( adapters, adapter ):
	if adapter.rating in branches_cache:
		return branches_cache[ adapter.rating ]
	
	count = adapter.num_branches
	
	for rating in adapter.connections:
		count += get_branches_recurse( adapters, adapters[ rating ] )
		
	branches_cache[ adapter.rating ] = count
...
class Adapter( ):
	...
	def __repr__( self ):
		return '<Adapter {0}>'.format( self.rating )
...
if __name__ == "__main__":
	...
	# Make sorted list of adapters, plus power port & device
	lines = open( INPUT_FILENAME, 'r' ).read( ).splitlines( )
	
	ratings = [ 0 ] + [ int( l ) for l in lines ]
	ratings.sort( )
	ratings.append( ratings[ -1 ] + 3 )
	
	# Make a dict of adapter objects
	# Each one has list of connections to other adapters
	adapters = { 0: Adapter( 0 ) }

	for rating in ratings:
		adapter = Adapter( rating )
		
		# Make connections to other adapters that aren't > 3 jolts away
		adapter.connections = [ r for r in ratings if r > rating and r - rating <= 3 ]
		
		# Pre-calculate # of new branches coming off this adapter
		# to save time in recursive loop later
		adapter.num_branches = max( 0, len( adapter.connections ) - 1 )

		adapters[ rating ] = adapter
			
	# Starting at power port, walk the adapters tree to count arrangements
	count = get_branches_recurse( adapters, adapters[ 0 ] ) + 1
	
	print( 'answer =', count )
	print( '{0:.5f} secs'.format( time.clock( ) - time_start ) )
