...
if __name__ == "__main__":
	...
	while True:
		...
		while i < len( lines ) and 'mask' not in lines[ i ]:
			print( 'line {0} of {1} ({2:.2f}%)'.format( i, len( lines ), i / len( lines ) * 100.0 ) )
			...
			parts = line.split( ' ' )
			value = int( parts[ -1 ] )
			addr_dec = int( parts[ 0 ].split( '[' )[ 1 ][ :-1 ] )
			
			# Convert memory decimal to binary string
			addr_bin_short = numpy.binary_repr( addr_dec )
			addr_bin = '0' * ( 36 - len( addr_bin_short ) ) + addr_bin_short
		
			print( 'addr   = {0}'.format( addr_bin ) )
			print( 'mask   = {0}'.format( mask ) )
			
			# Apply mask to value
			addr_list = [ ]
			
			for j in range( len( addr_bin ) ):
				if mask[ j ] == '0':
					addr_list.append( addr_bin[ j ] )
				elif mask[ j ] == '1':
					addr_list.append( '1' )
				else:
					addr_list.append( 'X' )

			print( 'appl   = {0}'.format( ''.join( addr_list ) ) )
			       
			# Accounting for floating bits, "X", permutate through all combinations,
			# writing the value to each of those memory addresses as we go.
			
			# Get mask indices where we have floating bits
			x_indices = [ j for j in range( 36 ) if addr_list[ j ] == 'X' ]

			# Generate all permutations of ( "0", "1" ) across the N indices we found
			combos = [ tuple( j ) for j in itertools.product( ( '0', '1' ), repeat = len( x_indices ) ) ]

			for combo in combos:
				# Apply this combo to our binary string
				for j in range( len( x_indices ) ):
					idx = x_indices[ j ]
					addr_list[ idx ] = combo[ j ]
