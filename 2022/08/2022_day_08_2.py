...
def get_scenic_score(data, coord) -> int:
	cy, cx = coord
	val = data[coord]
	height = data[cy, cx]
	
	# Lines of sight in 4 directions: N, E, S, W
	# Flip two of these so the first element is closest to our tree
	lines = [
		numpy.flip(data[:cy, cx:cx+1]),
		data[cy+1:, cx:cx+1:],
		numpy.flip(data[cy:cy+1, :cx]),
		data[cy:cy+1, cx+1:]
	]
...
if __name__ == '__main__':
	...
	for y in range(size_y):
		for x in range(size_x):
			max_score = max(max_score, get_scenic_score(data, (y, x)))
