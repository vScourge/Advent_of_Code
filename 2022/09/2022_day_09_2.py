...
def move_rope(rope):
	# The head has moved. Move each remaining knot to follow the one before it.
	# But only if distance between them is > 1
	for i in range(1, len(rope)):
		if abs(rope[i-1].x - rope[i].x) <= 1 and abs(rope[i-1].y - rope[i].y) <= 1:
			continue
...
if __name__ == '__main__':
	...
	for move in data:
		...
		for i in range(h_dist):
			...
			# Move each non-head knot
			rope = move_rope(rope)
		
			# Add to list of visited locations
			tail_positions.append((rope[-1].x, rope[-1].y))
