...
def get_dir_size_recurse(tree_data: list, directory: Directory) -> int:
	...
	# Now add size of each dir inside this dir
	sub_dirs = [d for d in tree_data if d.parent == directory]
	
	for sub_dir in sub_dirs:
		if sub_dir.size == 0:
			# Needs to be calculated
			sub_dir.size = get_dir_size_recurse(tree_data, sub_dir)
...
def parse_input() -> list:
	...
	for i in range(len(lines)):
		...
		if line == '$ cd ..':
			...
			tree_data.append(new_dir)
			cur_dir = new_dir
			
		elif line == '$ ls':
			# Reads forward to gather dir contents
			j = i+1
	...
	for directory in tree_data:
		directory.size = get_dir_size_recurse(tree_data, directory)
