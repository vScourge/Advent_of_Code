...
class Valve:
	def __init__(self, name, flow, link_names):
		self.name = name
		self.flow = flow
...
def parse_input( ) -> tuple:
	...
	for line in open('input0.txt', 'r'):
		...
		if ',' in part2:
			link_names = [s.strip() for s in part2.split('valves')[-1].split(',')]
		else:
			link_names = [part2.split(' ')[-1]]
			
		valves[name] = Valve(name, flow, link_names)
	
	# Connect links
	for valve in valves.values():
		for link_name in valve.link_names:
			valve.links.append(valves[link_name])
...
if __name__ == '__main__':
	...
	for name in valves:
		print(f'{name} = {valves[name]}, flow {valves[name].flow} - {valves[name].links}')
		
	# key = flow, value = valve
	flows = { }
	for valve in valves.values():
		if valve.flow not in flows:
			flows[valve.flow] = [ ]
			
		flows[valve.flow].append(valve)

	for flow in sorted(flows, reverse=True):
