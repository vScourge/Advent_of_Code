...
that your worry level after the monkey inspected the item is five times whatever your worry level was 
before inspection.)
...
def parse_input() -> list:
	...
	while i < len(lines):
		...
		monkey.operation = lines[i+1].split('=')[-1].replace('old', '{0}')
		monkey.test = int(lines[i+2].split(' ')[-1])
		monkey.targets = (int(lines[i+3].split(' ')[-1]), int(lines[i+4].split(' ')[-1]))
	...
	for round in range(20):
		for i in range(len(data)):
			...
			for item in monkey.items:
				...
				worry = int(eval(monkey.operation.format(item)) / 3)
				...
				data[t].items.append(worry)

			monkey.items = [ ]

	# Find two highest number of inspections per monkey
	active_monkeys = sorted([m.inspections for m in data.values()])
	answer = active_monkeys[-1] * active_monkeys[-2]
