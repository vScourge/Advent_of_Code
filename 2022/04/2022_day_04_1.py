...
would be exclusively cleaning sections their partner will already be cleaning, so these seem like the most in need 
...
for line in open('input.txt', 'r'):
	...
	split = assignment1.split('-')
	section1 = set(range(int(split[0]), int(split[1])+1))
	split = assignment2.split('-')
	section2 = set(range(int(split[0]), int(split[1])+1))
