...
def parse_input( ) -> tuple:
	...
	all_things = sensors + beacons
	
	min_x = min([p.x for p in all_things])
	max_x = max([p.x for p in all_things])
	min_y = min([p.y for p in all_things])
	max_y = max([p.y for p in all_things])
