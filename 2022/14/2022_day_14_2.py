...
def parse_input( ) -> tuple:
	...
	grid = numpy.full((maxy-miny+3, maxx-minx+1), fill_value = '.')
...
def simulate_sand(grid, pos):
	...
	while True:
		...
		# Check downward
		if pos.y == len(grid) - 1 and grid[pos.y+1][pos.x] == '#':
			# Sand has come to a rest
			grid[pos.y][pos.x] = 'o'
			pos = Point2(start.y, start.x)
			continue
		elif grid[pos.y+1][pos.x] == '.':
			pos.y += 1
			continue
		
		if pos.x-1 < 0:
			# off left edge of grid, so pad grid to expand
			column = numpy.array(['.' for i in range(len(grid))])
			grid = numpy.insert(grid, 0, column, axis = 1)
			# Extend floor
			grid[-1][0] = '#'
			# Adjust coords
			pos.x += 1
		...
		if pos.x+1 >= len(grid[0]):
			# off right edge of grid, so pad grid to expand
			column = numpy.array(['.' for i in range(len(grid))])
			grid = numpy.insert(grid, len(grid[0]), column, axis = 1)
			# Extend floor
			grid[-1][-1] = '#'
