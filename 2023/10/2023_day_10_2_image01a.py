...
TILE_CONNECTIONS = {
	'|': ((DIR_N, DIR_S), PIL.Image.open('tile_1.png')),
	'-': ((DIR_E, DIR_W), PIL.Image.open('tile_2.png')),
	'L': ((DIR_N, DIR_E), PIL.Image.open('tile_3.png')),
	'J': ((DIR_N, DIR_W), PIL.Image.open('tile_4.png')),
	'7': ((DIR_S, DIR_W), PIL.Image.open('tile_5.png')),
	'F': ((DIR_E, DIR_S), PIL.Image.open('tile_6.png')),
	'.': (tuple(), None),
	'S': ((DIR_E, DIR_S, DIR_N, DIR_W), PIL.Image.open('tile_7.png')),
}
...
class Tile():
	def __init__(self, y, x, char):
		...
		self.char = char
		self.start = False
		self.dirs = TILE_CONNECTIONS[char][0]
		self.sprite = TILE_CONNECTIONS[char][1]
		self.connections = [ ]
...
if __name__ == '__main__':
	...
	image = PIL.Image.new('RGB', (len(grid[0])*3, len(grid)*3))
	...
	for y in range(len(grid)):
		for x in range(len(grid[0])):
			...
			if tile in loop:
				image.paste(tile.sprite, (x*3, y*3))
	...
	# Here is where I used Photoshop to floodfill the "interior", then re-color every 3x3 block inside the interior
	# a different color (58, 195, 0). The following code then loaded that image and counted those pixels, then
	# I divided by 9.
	
	count = 0
	
	image = PIL.Image.open('grid2.png')
	
	for y in range(image.height):
		for x in range(image.width):
			if image.getpixel((x,y)) == (58,154,0):
				count += 1
	
	print(f'count = {count}')
	print(f'answer = {count/9}')
