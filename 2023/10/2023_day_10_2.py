...
if __name__ == '__main__':
	...
	# Now go through entire grid, row by row, to count # of tiles inside the main loop. If we cross the loop,
	# start counting tiles until we cross it a second time.
	loop = loop_longest
	in_loop = False
	on_loop = False
	count = 0
	...
	for y in range(len(grid)):
		for x in range(len(grid[0])):
			...
			if tile in loop and last_tile.char == '|' and tile.char == '|':
				in_loop = not in_loop
				continue
