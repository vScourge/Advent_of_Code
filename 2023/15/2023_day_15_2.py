...
hard hat. The reindeer leads you to a shelf of goggles and hard hats (you quickly find some that fit) and then further 
into the facility. At one point, you pass a button with a faint snout mark and the label "PUSH FOR HELP". No wonder 
...
class Lens():
	def __init__(self, label, focal):
		self.label = label
		self.focal = focal
		self.hash = get_hash(label)

	def __repr__(self):
		return f'<Lens "{self.label}" {self.focal}>'
