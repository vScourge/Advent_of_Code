...
but the machines have broken down because Desert Island recently stopped receiving the parts they need to 
fix the machines.
...
class Hand():
	def __init__(self, cards: str, bid: int):
		...
		self.card_values = self._get_card_values()
		self.type = self._get_type(self.card_values)
		self.value = HAND_VALUES[self.type]
	
		
	def _get_base_type(self, cards):
		if cards.count(cards[0]) == 5:
			return 'five of a kind'
		elif 4 in [cards.count(c) for c in cards]:
			return 'four of a kind'
		elif len(set(cards)) == 2:
			return 'full house'
		elif 3 in [cards.count(c) for c in cards] and len(set(cards)) == 3:
			return 'three of a kind'
		elif [cards.count(c) for c in cards].count(2) == 4:
			return 'two pair'
		elif [cards.count(c) for c in cards].count(2) == 2:
			return 'one pair'
		elif len(set([cards.count(c) for c in cards])) == 1:
			return 'high card'
		
	
	def _get_type(self, cards):
		best_hand_type = self._get_base_type(cards)
		
		if 1 not in cards:
			return best_hand_type
		
		best_hand_value = HAND_VALUES[best_hand_type]
		
		vals = set([v for v in cards if v != 1])
		...
		for val in vals:
			...
			new_hand_type = self._get_base_type(cards2)
			new_hand_value = HAND_VALUES[new_hand_type]
	...
	def _get_card_values(self):
		...
		for card in self.cards:
			if card.isdigit():
				...
				vals.append(FACE_CARD_VALUES[card])
		...
		if other.value > self.value:
			...
			for i in range(5):
				if other.card_values[i] > self.card_values[i]:
					return True
				elif other.card_values[i] < self.card_values[i]:
					return False
	...
	def __repr__(self):
		return '<Hand "{0}", {1}>'.format(''.join(self.cards), self.value)
...
def parse_input():
	...
	for line in open('input.txt', 'r'):
		...
		data.append(([c for c in cards], int(bet)))
		
	return data


def get_hands_winnings(hands):
	winnings = [ ]
	i = 1
	
	for hand in sorted(hands):
		winnings.append(i * hand.bid)
		i += 1
...
if __name__ == '__main__':
	...
	hands = [Hand(x[0], x[1]) for x in data]
	
	winnings = get_hands_winnings(hands)	
	
	print('winnings =', sum(winnings))
