...
class Hand():
	...
	def _get_type(self):
		...
		elif 4 in [self.cards.count(c) for c in self.cards]:
			return 'four of a kind'
		elif len(set(self.cards)) == 2:
			return 'full house'
		elif 3 in [self.cards.count(c) for c in self.cards] and len(set(self.cards)) == 3:
			return 'three of a kind'
		elif [self.cards.count(c) for c in self.cards].count(2) == 4:
			return 'two pair'
		elif [self.cards.count(c) for c in self.cards].count(2) == 2:
			return 'one pair'
		elif len(set([self.cards.count(c) for c in self.cards])) == 1:
			return 'high card'
