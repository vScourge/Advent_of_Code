...
def parse_input():
	...
	for line in open('input.txt', 'r'):
		...
		for part in parts:
			entries.append(set([int(x.strip()) for x in part.split(' ') if x]))
...
if __name__ == '__main__':
	total = 0
	data = parse_input()
	stack: list[int] = [*range(len(data))]
	
	while stack:
		card_idx = stack.pop(0)
		total += 1

		winners, numbers = data[card_idx]
		match_count = len(winners.intersection(numbers))

		if match_count:
			val = [*range(card_idx + 1, card_idx + 1 + match_count)]
			stack = [*val, *stack]
