...
def parse_input():
	...
	entries = [int(x.replace(' ', '')) for x in lines]
...
if __name__ == '__main__':
	...
	# First find the min hold time that beats record
	for race in data:
		time, record = race
	...
	# Now find the max hold time that beats record
	for race in data:
		time, record = race
	...
	print('total =', max_hold - min_hold + 1)
