...
if __name__ == '__main__':
	...
	for game_id, game in data.items():
		...
		for cube_set in game:
			min_cubes[0] = max(min_cubes[0], cube_set[0])
			min_cubes[1] = max(min_cubes[1], cube_set[1])
			min_cubes[2] = max(min_cubes[2], cube_set[2])

		powers = min_cubes[0] * min_cubes[1] * min_cubes[2]
		total += powers
