...
def process_step( data, step ):
	# Track which values have flashed this step
	flashed = [ ]
	
	# Increment all by one
	all_real_vals = data[ 1:11, 1:11 ]
	all_real_vals += 1
	
	# Flash any that are > 9
	do_next_step = True
		...
		for y in range( 1, 11 ):
			for x in range( 1, 11 ):
				...
				if val > 9 and (x,y) not in flashed:
					...
					# Get slice around this value
					y_start = max( 1, y-1 )
					y_end = min( 11, y+2 )
					x_start = max( 1, x-1 )
					x_end = min( 11, x+2 )
					
					adj_vals = data[ y_start:y_end, x_start:x_end ]
					adj_vals += 1
					
					flashed.append( (x,y) )
					
	# Reset any that flashed to zero
	for x,y in flashed:
		data[ y, x ] = 0
...
for step in range( 10000 ):
	# Print our board
	print( '\nAfter step {0}:'.format( step + 1 ) )
	
	print_grid( data )
		
	step_flashes = process_step( data, step )
	total_flashed += step_flashes
	
	print( 'flashes =', total_flashed )
