...
horizontal position costs 1 more unit of fuel than the last: the first step costs 1, the second step costs 2, the third step costs 3, and so on.
...
for goal in range( min( data ), max( data ) + 1 ):
	fuels = [ calculate_fuel( x, goal ) for x in data ]
	fuel = sum( fuels )

	all_fuels.append( fuel )
	
answer = min( all_fuels )	
