...
def filter_data( numbers, mode ):
	...
	while len( data ) > 1:
		...
		for i in range( num_cols ):
			bits = find_common_bits( data, mode )
			keep = [ ]
...
def find_common_bits( numbers, mode ):
	# Build a list of the most or least common bits in each column
	bits = ''
	
	for i in range( len( numbers[ 0 ] ) ):
		col_bits = [ n[ i ] for n in numbers ]

		bincount = numpy.bincount( col_bits )

		if len( bincount ) == 2 and bincount[ 0 ] == bincount[ 1 ]:
			if mode == MOST:
				common_bit = '1'
			else:
				common_bit = '0'
			...
			if mode == MOST:
				common_bit = numpy.argmax( bincount )
			else:
				common_bit = numpy.argmin( bincount )
...
numbers = [ line.strip( ) for line in open( 'input.txt', 'r' ).readlines( ) ]

# Find oxygen
data = list( numbers )

oxygen_number = filter_data( data, MOST )
oxygen_number = int( oxygen_number, 2 )

co2_number = filter_data( data, LEAST )
co2_number = int( co2_number, 2 )

print( 'oxygen_number =', oxygen_number)
print( 'co2_number =', co2_number )
print( 'answer =', oxygen_number * co2_number )
