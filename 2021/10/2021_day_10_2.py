...
was found once (1197 points), and an illegal > was found once (25137 points). So, the total syntax error score for this file 
...
You can only use closing characters (), ], }, or >), and you must add them in the correct order so that only legal pairs are 
...
def validate_data( lines ):
	# Find corrupted lines and discard them
	data = [ ]
	score = 0
...
def fix_incomplete_lines( data ):
	...
	for line in data:
		...
		# At end of line, so figure out closing chars needed to complete it
		close_chars = ''
		
		for open_char in reversed( stack ):
			close_chars += chars[ open_char ]
			
		for c in close_chars:
			score *= 5
			score += scores_map[ c ]
		
		print( close_chars, score )
		scores.append( score )
...
scores = fix_incomplete_lines( data )
