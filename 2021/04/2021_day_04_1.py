...
number is marked on all boards on which it appears. (Numbers may not appear on all boards.) If all numbers in any row or any 
column of a board are marked, that board wins. (Diagonals don't count.)
...
def parse_input( ):
	...
	moves = [ int( x ) for x in lines[ 0 ].strip( ).split( ',' ) ]
	...
	while i <= len( lines ):
		for r in range( 5 ):
			row = [ int( x ) for x in lines[ i ].strip( ).split( ' ' ) if x ]
			rows.append( row )
			i += 1
	
		boards.append( numpy.array( rows ) )
		rows = [ ]
		i += 1
...
def check_for_bingo( board, drawn ):
	# Check rows
	for row in board:
		if all( numpy.in1d( row, drawn ) ):
			return True
		
	# Check columns
	for col in range( 5 ):
		vals = board[ :,col ]
		if all( numpy.in1d( vals, drawn ) ):
			return True
...
drawn = numpy.array( [ ], dtype = numpy.int64 )
c = 0

for move in moves:
	#print( 'move =', c )
	c += 1
	drawn = numpy.append( drawn, int( move ) )
	
	if len( drawn ) >= 5:
		for board in boards:
			winner = check_for_bingo( board, drawn )
			
			if winner:
				#print( 'WINNER' )
				unmarked = [ x for x in board.ravel( ) if x not in drawn ]
				score = sum( unmarked ) * move
				print( 'score =', score )
