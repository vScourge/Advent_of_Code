...
def pathfind( start, goal ):
	...
	while True:
		...
		for link in cur_path[ -1 ].links:
			if link.size == SMALL:
				small_visited_twice = any( [ x for x in visited_smalls.values( ) if x > 1 ] )
				
				if link in visited_smalls:
					if link.name in ( 'start', ) or small_visited_twice:
						continue
					else:
						visited_smalls[ link ] += 1
...
for path in paths:
	val = ','.join( [ c.name for c in path ] )
	path_names.append( val )
