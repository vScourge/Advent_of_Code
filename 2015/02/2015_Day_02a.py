...
--- Part Two ---
The elves are also running low on ribbon. Ribbon is all the same width, so they only have to worry about the length they need to order, which they would
again like to be exact.

The ribbon required to wrap a present is the shortest distance around its sides, or the smallest perimeter of any one face. Each present also requires a
bow made out of ribbon as well; the feet of ribbon required for the perfect bow is equal to the cubic feet of volume of the present. Don't ask how they
...
if __name__ == "__main__":
	...
	for line in open( 'input.txt', 'r' ):
		...
		area = sum( [ 2*x for x in sides ] ) + smallest_side
		total_paper += area

		dims = sorted_dims[ :2 ]
		total_ribbon += dims[ 0 ] * 2 + dims[ 1 ] * 2 + l*w*h


	print( 'total paper = {0} sq/ft'.format( total_paper ) )
	print( 'total ribbon = {0} ft'.format( total_ribbon ) )
